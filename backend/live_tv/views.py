...
class ChannelViewSet(viewsets.ModelViewSet):
    ...
    Any user: read-only, filterable by ?country_slug=<slug>&category=<slug_or_name>.
    """
    serializer_class = ChannelSerializer
    permission_classes = [IsAdminOrReadOnly]

    def get_queryset(self):
        queryset = Channel.objects.filter(is_active=True).select_related("country", "category")

        # 1. Filter by Country Slug
        ...
        # 2. Filter by Category Name (No Q import needed)
        category_param = self.request.query_params.get("category", None)
        if category_param and category_param != 'all':
            # Handles either case-insensitive name filtering directly through relations
...
class YouTubeStreamResolverView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        proxy_dict = {"http": production_proxy,
                      "https": production_proxy} if production_proxy else {}
        ...
        if segment_url:
            ...
            try:
                response = requests.get(
                    segment_url, proxies=proxy_dict, stream=True, timeout=15)
                django_stream = HttpResponse(
        ...
        try:
            ...
            modified_playlist = re.sub(
                r'https?://[^\s]+googlevideo\.com[^\s]+', replace_link, playlist_content)

            django_response = HttpResponse(
                modified_playlist, content_type="application/x-mpegURL")
...
class BioscopePremiumStreamView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            response = requests.get(
                full_url, headers=premium_headers, timeout=10)

            if response.status_code != 200:
                print(
                    f"❌ Upstream Manifest failed with status: {response.status_code}")
            ...
            for line in m3u8_content.splitlines():
                ...
                if "ivy.bioscopelive.com" in clean_line or (clean_line.startswith('http') and '.ts' in clean_line):
                    if 'URI="' in clean_line:
                        ...
                        modified_line = clean_line.replace(
                            absolute_url, proxied_url)
...
class BioscopeSegmentRelayView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            proxied_res = requests.get(
                target_ts_url, headers=premium_headers, stream=True, timeout=15)

            return StreamingHttpResponse(
                proxied_res.iter_content(chunk_size=8192),
                status=proxied_res.status_code,
                content_type=proxied_res.headers.get(
