...
const Users = () => {
    ...
    const fetchUsers = async (pageProp, searchProp, limitProp) => {
        setLoading(true);
        try {
    ...
    const debouncedFetch = (search) => {
        if (timeoutId) clearTimeout(timeoutId);
        const id = setTimeout(() => {
            setPage(1);
            fetchUsers(1, search, showBy);
        }, 500);
        setTimeoutId(id);
    };

    useEffect(() => {
        fetchUsers(page, searchQuery, showBy);
    }, [page, showBy]);
        ...
        <div className="p-4 sm:p-8 min-h-screen bg-transparent">
            ...
            <div className="mt-8 px-4 sm:px-0 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                    Registry contains <span className="text-blue-400">{totalCount}</span> verified personnel
                </div>
