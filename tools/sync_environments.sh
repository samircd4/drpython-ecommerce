...
# 1a. Wipe the destination schema first for a truly clean sync
echo "  🧹 Wiping destination schema (public)..."
docker exec -i $DEST_DB_CONT psql -U $DEST_DB_USER -d $DEST_DB_NAME -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;" > /dev/null

# 1b. Restore from source
# Added --no-owner and --no-privileges to avoid permission errors
